        ...
        <section className="mt-6 py-4 px-16 overflow-hidden">
            ...
            <div className="flex gap-4 relative items-center w-full group">
                ...
                <div className="flex gap-4 transition-transform duration-500" style={{transform: `translateX(-${index * moveBy}px)`}}>
                    {
                        movie.map(card => (
                            <div key={card.id} onClick={() => navigate(`/${card.id}`)} onMouseEnter={() => setActive(card.id)} onMouseLeave={() => setActive(null)}  className="flex-shrink-0 w-70 h-40 bg-black rounded-[6px] mt-4 cursor-pointer overflow-hidden relative">
